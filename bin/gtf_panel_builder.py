...
class CDSOrderingEngine:
    ...
    def build_cds_index(self, gene_id, transcript_id):
        ...
        if df["cds_index"].duplicated().any():
            raise ValueError(
                f"Duplicate exon_number detected in {transcript_id}"
            )
    ...
    def _get_cds(self, gene_id, transcript_id):
        return self.cds[
            (self.cds.gene_id == gene_id) &
            (self.cds.transcript_id == transcript_id)
        ]
...
# Write BED file
bed.df.to_csv(args.out, sep="\t", header=False, index=False, 
